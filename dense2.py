...
def add_transition_layer_to_classes(inputs, is_training, final_pool_x, final_pool_y):
    ...
    padding = 'VALID'
    output = tf.nn.max_pool(output, window, strides, padding)
    # *************** TODO
    return output
        ...
        with tf.variable_scope("Block_" + str(i)):
            for j in range(layers_per_block):
                with tf.variable_scope("Inner_Layer_" + str(j)):
                    densenet = add_internal_layer(densenet, g[0][j], is_training, dropout_rate, b_mode, is_audio)
